...
// Test PostScript processing functions
int LLVMFuzzerTestOneInput(const uint8_t *Data, size_t Size)
{
    if (Size < 10 || Size > 300000)
    {
        return 0;
    }
    ...
    char input_file[] = "/tmp/fuzz_ps_input_XXXXXX";
    ...
    char output_file[] = "/tmp/fuzz_ps_output_XXXXXX";
    ...
    // Test ppdFilterPSToPS
    int result = ppdFilterPSToPS(input_fd, output_fd, 1, &data, NULL);
    ...
    // Test cfFilterGhostscript with PostScript input to PDF
    ...
    // Test cfFilterGhostscript with PostScript input to Raster
    lseek(input_fd, 0, SEEK_SET);
    lseek(output_fd, 0, SEEK_SET);
    ...
    // Test cfFilterGhostscript with PostScript input to PCL-XL
    lseek(input_fd, 0, SEEK_SET);
    lseek(output_fd, 0, SEEK_SET);
    outformat = CF_FILTER_OUT_FORMAT_PXL;
    data.final_content_type = "application/vnd.hp-pclxl";
    result = cfFilterGhostscript(input_fd, output_fd, 1, &data, &outformat);
    (void)result;

    // Test cfFilterGhostscript with PostScript input to CUPS Raster
    lseek(input_fd, 0, SEEK_SET);
    lseek(output_fd, 0, SEEK_SET);
    ...
    // Test cfFilterGhostscript with PostScript input to Apple Raster
    lseek(input_fd, 0, SEEK_SET);
    lseek(output_fd, 0, SEEK_SET);
    outformat = CF_FILTER_OUT_FORMAT_APPLE_RASTER;
    data.final_content_type = "image/urf";
    result = cfFilterGhostscript(input_fd, output_fd, 1, &data, &outformat);
    (void)result;

    // Test cfFilterGhostscript with PostScript input to PCLM
    lseek(input_fd, 0, SEEK_SET);
    lseek(output_fd, 0, SEEK_SET);
